...
- 인터페이스에 원하는 필드의 getXxx()만 작성하여 사용할 수 있다.
- 클래스 기반의 DTO 형식도 가능하다.
- 제너릭 타입을 활용해 동적으로 프로젝션도 가능하다.
- 중첩 구조 사용 시 주의사항
  - 프로젝션 대상이 root엔티티면 JPQL SELECT 최적화 가능
  - root엔티티가 아니라면 무조건 LEFT OUTER JOIN 처리가 되고 모든 필드를 우선 조회한 다음 DTO 형식으로 반환된다.
...
- 페이징 지원한다.
- 반환 타입이 거의 다 지원된다.
- Sort 파라미터를 통한 정렬이 정상 작동하지 않을 수 있다.
- JPQL처럼 어플리케이션 로딩 시점에 파싱을 통해 문법 확인이 불가능하다.
- 동적 쿼리가 불가능하다.
- Page 사용 가능, DTO 반환 결합 가능 (SELECT 절에 원하는 필드만 조회 가능)

> 원하는 필드만 조회할 경우는 쿼리문이 복잡하고 성능 최적화가 필요한 경우이므로 필요할 때 적절히 사용하자
> 
> 동적 쿼리를 활용할 때는 QueryDSL을 사용하자!